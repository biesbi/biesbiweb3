...
class RateLimit
{
    ...
    public static function check(string $endpoint, int $maxHits = 10, int $windowSec = 300): void
    {
        ...
        if (!$row) {
            // İlk istek — aynı anda gelen paralel isteklerde duplicate key olabilir.
            try {
                $pdo->prepare(
                ...
                // Unique çakışması: kayıt bu anda başka request tarafından açılmış.
                // Akışı mevcut kayıt gibi devam ettir.
