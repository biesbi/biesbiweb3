...
switch ($id) {
    case 'login':
        ...
        // Sütun kontrollerini tek seferde yap (her biri information_schema sorgusu)
        $hasPasswordHash  = tableHasColumn('users', 'password_hash');
        $hasUsername      = tableHasColumn('users', 'username');
        $hasEmailVerified = tableHasColumn('users', 'email_verified_at');
        $hasLastLogin     = tableHasColumn('users', 'last_login');

        if (!$hasLastLogin) {
            db()->exec('ALTER TABLE users ADD COLUMN last_login DATETIME NULL');
            // Şema değişti — önbelleği temizle
            $cacheFile = sys_get_temp_dir() . '/schema_cache_' . md5(DB_NAME) . '.php';
            @unlink($cacheFile);
            unset($GLOBALS['_schema_file_cache']);
        }
