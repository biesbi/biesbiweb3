...
final class PaytrService {
    ...
    public static function createPayment(array $order, array $items, array $shippingAddress): array {
        try {
            ...
            return [
                ...
                'message' => $e->getMessage() !== '' ? $e->getMessage() : 'PAYTR baslatilamadi.',
            ];
        }
    }
    ...
    public static function completeMock(string $orderId, string $status): array {
        ...
        if (tableHasColumn('orders', 'payment_reference')) {
            db()->prepare('UPDATE orders SET payment_reference = ? WHERE id = ?')
                ->execute([$orderId, $orderId]);
        }
        if (tableHasColumn('orders', 'paid_at') && $status === 'success') {
            db()->prepare('UPDATE orders SET paid_at = CURRENT_TIMESTAMP WHERE id = ?')
                ->execute([$orderId]);
        }
        if (tableHasColumn('orders', 'failed_at') && $status !== 'success') {
            db()->prepare('UPDATE orders SET failed_at = CURRENT_TIMESTAMP WHERE id = ?')
                ->execute([$orderId]);
        }
    ...
    public static function handleCallback(array $data): never {
        ...
        if (tableHasColumn('orders', 'payment_reference')) {
            db()->prepare('UPDATE orders SET payment_reference = ? WHERE id = ?')
                ->execute([$merchantOid, $merchantOid]);
        }
        if (tableHasColumn('orders', 'paid_at') && $status === 'success') {
            db()->prepare('UPDATE orders SET paid_at = CURRENT_TIMESTAMP WHERE id = ?')
                ->execute([$merchantOid]);
    ...
    private static function createIframePayment(array $order, array $items, array $shippingAddress): array {
        ...
        $payload = [
            ...
            'merchant_ok_url' => rtrim((string) env('APP_URL', ''), '/') . '/paytr-result.html?status=success',
            'merchant_fail_url' => rtrim((string) env('APP_URL', ''), '/') . '/paytr-result.html?status=failed',
        ...
        if (($response['status'] ?? '') !== 'success' || empty($response['token'])) {
            return [
                ...
                'status' => 'failed',
                'merchant_oid' => $merchantOid,
                'message' => (string) ($response['reason'] ?? $response['err_msg'] ?? 'PAYTR token olusturulamadi.'),
                'response' => $response,
            ];
        }
    ...
    private static function resolveUserIp(): string {
        ...
        foreach ($candidates as $candidate) {
            $ip = self::extractFirstIp((string) $candidate);
        ...
        if (self::useMock() || self::isTestMode()) {
            return '127.0.0.1';
        }
    ...
    private static function extractFirstIp(string $value): string {
