...
// Tüm upload işlemleri admin yetkisi gerektirir
$admin = null;
if (!(($id === 'category-banner' && $sub === null && $method === 'GET')
    || ($id === 'product-file' && $sub !== null && $method === 'GET'))) {
...
if ($id === 'product-image' && $sub === null && $method === 'POST') {
    ...
    // Ürün var mı?
    $pChk = db()->prepare('SELECT id FROM products WHERE id COLLATE utf8mb4_turkish_ci = ? LIMIT 1');
...
// ─── GET /upload/product-image/{product_id} ──
...
// ─── GET /upload/category-banner ─────────────
elseif ($id === 'category-banner' && $sub === null && $method === 'GET') {
    ok(UploadService::getCategoryBannerManifest());
}

// ─── GET /upload/product-file/{filename} ─────────────────────
elseif ($id === 'product-file' && $sub !== null && $method === 'GET') {
    UploadService::outputProductImageFile((string) $sub);
}

// ─── POST /upload/category-banner/{slot} ─────
elseif ($id === 'category-banner' && $sub !== null && $method === 'POST') {
    if (empty($_FILES['image'])) error('Dosya alani "image" adiyla gonderilmeli.');
    $result = UploadService::saveCategoryBanner(
        $_FILES['image'],
        (string) $sub,
        isset($admin['sub']) ? (string) $admin['sub'] : null
    );
    ok($result, 'Kategori banner gorseli guncellendi.');
}
...
elseif ($id === 'product-image' && is_numeric($sub) && $method === 'PATCH') {
    ...
    if (input('is_primary') === true || input('is_primary') === 'true' || input('is_primary') === 1) {
        ...
        db()->prepare('UPDATE product_images SET is_primary = 0 WHERE product_id COLLATE utf8mb4_turkish_ci = ?')
    ...
    db()->prepare('UPDATE product_images SET ' . implode(', ', $updates) . ' WHERE id = ?')
        ->execute($values);
    UploadService::syncProductImageFields((string) $image['product_id']);
