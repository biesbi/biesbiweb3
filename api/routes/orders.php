...
function orderColumnExists(string $column): bool {
    return tableHasColumn('orders', $column);
}

function orderItemColumnExists(string $column): bool {
    return tableHasColumn('order_items', $column);
}
...
if ($id === null && $method === 'POST') {
    ...
    $freeShippingApplied = $shippingFee <= 0.0;
    ...
    try {
        $orderColumns = ['id', 'user_id', 'shipping_address', 'subtotal'];
        $orderValues = [
            $orderId,
            $payload['sub'] ?? null,
        ...
        $shippingCostColumn = orderColumnExists('shipping_cost') ? 'shipping_cost' : 'shipping_fee';
        $orderColumns[] = $shippingCostColumn;
        $orderValues[] = $shippingFee;

        foreach ([
            'shipping_total_desi' => (int) ceil($totalDesi),
            'free_shipping_applied' => $freeShippingApplied ? 1 : 0,
            'shipping_rule_snapshot' => json_encode($shippingCalculation['group'] ?? null, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES),
            'discount' => $discount,
        ...
        foreach ($items as $item) {
            $productSnapshot = ProductCatalogService::enrichProduct($item['product']);
            $itemColumns = ['order_id', 'product_id', 'product_name'];
            $itemValues = [
                $orderId,
            ...
            $itemColumnMap = [
                'product_slug' => $productSnapshot['slug'] ?? null,
                'product_img' => $productSnapshot['img'] ?? null,
                'product_image' => $productSnapshot['img'] ?? null,
                'price' => $item['price'],
                'unit_price' => $item['price'],
                'quantity' => $item['quantity'],
                'desi' => $item['desi'],
                'product_condition' => $productSnapshot['productCondition'] ?? 'new',
                'pieces' => $productSnapshot['pieces'],
                'category_name' => $productSnapshot['category'] ?? null,
                'brand_name' => $productSnapshot['brand'] ?? null,
            ];

            foreach ($itemColumnMap as $column => $value) {
                if (!orderItemColumnExists($column)) {
                    continue;
                }
                $itemColumns[] = $column;
                $itemValues[] = $value;
            ...
            $pdo->prepare(
                'INSERT INTO order_items (' . implode(', ', $itemColumns) . ')
